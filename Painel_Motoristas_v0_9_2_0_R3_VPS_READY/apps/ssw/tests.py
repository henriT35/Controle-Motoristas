...
class RetentionAttemptRegressionV0810Tests(TestCase):
    def test_rom34_beats_repeated_ctrc34_when_choosing_origin_attempt(self):
        with TemporaryDirectory() as tmp:
            path = Path(tmp) / 'retention-origin-attempt.sswweb'
            rows = [
                make_row(
                    ctrc='RET-ORIGIN-ATT', manifest='BEL-OLD-ATT', emission='01/09/2026',
                    driver='MOTORISTA ANTIGO', cpf='11111111111',
                    rom_code='85', rom_desc='SAIDA PARA ENTREGA', rom_date='01/09/2026', rom_time='08:00',
                    cte_code='34', cte_desc='MERCADORIA EM CONFERENCIA NO CLIENTE',
                    cte_date='01/09/2026', cte_time='12:00',
                ),
                make_row(
                    ctrc='RET-ORIGIN-ATT', manifest='BEL-REAL-34', emission='01/09/2026',
                    driver='MOTORISTA REAL', cpf='22222222222',
                    rom_code='34', rom_desc='MERCADORIA EM CONFERENCIA NO CLIENTE',
                    rom_date='01/09/2026', rom_time='11:00',
                    cte_code='34', cte_desc='MERCADORIA EM CONFERENCIA NO CLIENTE',
                    cte_date='01/09/2026', cte_time='12:00',
                ),
            ]
            write_file(path, rows, period='01/09/26 A 01/09/26')
            import_ssw_delivery_file(path)
            proof = RetainedProof.objects.get(cte__ctrc='RET-ORIGIN-ATT')
            self.assertEqual(proof.original_manifest.number, 'BEL-REAL-34')
            self.assertEqual(proof.original_driver.name, 'MOTORISTA REAL')

    def test_nonconclusive_ctrc_status_after_rom34_goes_to_tracking(self):
        cases = [("60", "DOCUMENTOS"), ("53", "AVARIA"), ("91", "INDENIZACAO")]
        for code, description in cases:
            with self.subTest(code=code), TemporaryDirectory() as tmp:
                path = Path(tmp) / f'retention-tracking-{code}.sswweb'
                write_file(path, [make_row(
                    ctrc=f'RET-TRACK-{code}', manifest=f'BEL-TRACK-{code}', emission='10/08/2026',
                    rom_code='34', rom_desc='MERCADORIA EM CONFERENCIA NO CLIENTE',
                    rom_date='10/08/2026', rom_time='11:00',
                    cte_code=code, cte_desc=description, cte_date='11/08/2026', cte_time='09:00',
                )])
                import_ssw_delivery_file(path)
                proof = RetainedProof.objects.get(cte__ctrc=f'RET-TRACK-{code}')
                self.assertEqual(proof.status, RetainedProof.Status.TRACKING)
                self.assertIsNone(proof.recovered_at)

    def test_tracking_becomes_recovered_when_current_ctrc_is_delivered(self):
        with TemporaryDirectory() as tmp:
            first = Path(tmp) / 'tracking.sswweb'
            delivered = Path(tmp) / 'delivered.sswweb'
            write_file(first, [make_row(
                ctrc='RET-TRACK-DEL', manifest='BEL-TRACK-DEL', emission='10/08/2026',
                rom_code='34', rom_desc='MERCADORIA EM CONFERENCIA NO CLIENTE',
                rom_date='10/08/2026', rom_time='11:00',
                cte_code='60', cte_desc='DOCUMENTOS', cte_date='11/08/2026', cte_time='09:00',
            )])
            write_file(delivered, [make_row(
                ctrc='RET-TRACK-DEL', manifest='BEL-TRACK-DEL', emission='10/08/2026',
                rom_code='34', rom_desc='MERCADORIA EM CONFERENCIA NO CLIENTE',
                rom_date='10/08/2026', rom_time='11:00',
                cte_code='1', cte_desc='ENTREGUE', cte_date='12/08/2026', cte_time='15:00',
            )])
            import_ssw_delivery_file(first)
            self.assertEqual(
                RetainedProof.objects.get(cte__ctrc='RET-TRACK-DEL').status,
                RetainedProof.Status.TRACKING,
            )
            import_ssw_delivery_file(delivered)
            proof = RetainedProof.objects.get(cte__ctrc='RET-TRACK-DEL')
            self.assertEqual(proof.status, RetainedProof.Status.RECOVERED)
            self.assertEqual(proof.recovered_at.date().isoformat(), '2026-08-12')
