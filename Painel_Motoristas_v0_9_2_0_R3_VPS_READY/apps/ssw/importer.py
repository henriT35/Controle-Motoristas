...
def _upsert_retained_proof(
    ...
    # v0.9.2: o estado consolidado atual do CTRC governa se a retenção continua
    # acionável. Datas podem ser corrigidas retroativamente pelo SSW e retained_at
    # pode conter horário técnico; por isso não usamos comparação temporal para
    # negar uma baixa quando o CTRC atual está 1/ENTREGUE.
    delivered_after_retention = is_delivered_occurrence(latest_code, latest_desc)
    active_retention = is_retention_occurrence(latest_code, latest_desc)
    tracking_state = bool(
        (latest_code or latest_desc)
        and not active_retention
        and not delivered_after_retention
    )
    row_has_rom_retention = is_retention_occurrence(snapshot.rom_code, snapshot.rom_description)
    ...
    proof, created = RetainedProof.objects.get_or_create(
        ...
        defaults={
            ...
            "volumes": max(parse_int(row.get("QTDE VOL")), 0),
            "status": (
                RetainedProof.Status.RECOVERED if delivered_after_retention
            ...
            "resolution_source": "SSW" if delivered_after_retention else "",
            "last_ssw_code": latest_code,
            "last_ssw_description": latest_desc,
            ...
            "note": (
                ...
                if delivered_after_retention and latest_at is not None
                else "[SSW AUTO] Baixa automática pelo estado consolidado do CTRC (data não informada)."
                if delivered_after_retention
                else "[SSW AUTO] Acompanhando alteração do SSW pelo estado consolidado do CTRC." if tracking_state
                else ""
            ),
        },
    )
    ...
    if retained_at and retained_at < proof.retained_at:
        ...
        changed = True
    if row_has_rom_retention and (
        proof.original_driver_id != driver.pk or proof.original_manifest_id != manifest.pk
    ):
    ...
    if not manual_recovery and proof.status != RetainedProof.Status.CANCELED:
        if delivered_after_retention:
            ...
            # O estado consolidado atual 34 reativa uma baixa AUTOMÁTICA, mesmo
            # quando as datas foram corrigidas retroativamente. Recuperação manual
            # validada já foi protegida por manual_recovery acima.
            if proof.status in {RetainedProof.Status.RECOVERED, RetainedProof.Status.VERIFY, RetainedProof.Status.TRACKING}:
                if proof.status != RetainedProof.Status.RECOVERED or proof.note.startswith("[SSW AUTO]"):
                    proof.status = RetainedProof.Status.WAITING
                    proof.recovered_at = None
                    proof.recovery_driver = None
                    proof.confirmed_by = None
                    proof.resolution_source = ""
                    proof.note = "[SSW AUTO] Retenção confirmada pelo estado consolidado do CTRC."
                    changed = True
        elif tracking_state:
            # Estados 60/53/91/etc. não provam recuperação nem retenção ativa.
            # Acompanhamos automaticamente e retiramos o item das ações do motorista.
            if proof.status != RetainedProof.Status.RECOVERED or proof.note.startswith("[SSW AUTO]"):
                if proof.status != RetainedProof.Status.TRACKING:
                    ...
                    proof.confirmed_by = None
                    proof.resolution_source = ""
                    proof.note = "[SSW AUTO] Acompanhando alteração do SSW pelo estado consolidado do CTRC."
                    changed = True
    if changed:
        proof.save()
    return False
...
def import_ssw_delivery_file(
    ...
    *,
    ...
    try:
        ...
        # Mesmo no engine legado, materializa a avaliação ao redor do importador
        # sem alterar robot_ssw. O comportamento é idempotente.
        try:
            ...
            sync_quality_events_for_movements(start=start_date, end=end_date)
            sync_retention_obligations(start=start_date, end=end_date)
            ...
            invalidate_operational_cache("ssw-legacy-import-completed")
            from apps.core.warmup import warm_navigation_cache
            warm_navigation_cache()
        except Exception:
            # O import legado preserva sua compatibilidade; o scheduler/startup
            # executa a mesma sincronização novamente e registra erros nos logs.
            pass
        return run, stats
