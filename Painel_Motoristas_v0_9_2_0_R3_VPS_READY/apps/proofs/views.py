...
@login_required
def index(request):
    timer = PerfTimer("proofs")
    # GET somente consulta o estado materializado. O matching das oportunidades
    # ocorre após importação/scheduler para não bloquear a troca de tela.
    ...
    kpis = filtered_qs.aggregate(
        ...
        verify=Count("id", filter=Q(status=RetainedProof.Status.VERIFY)),
        tracking=Count("id", filter=Q(status=RetainedProof.Status.TRACKING)),
    ...
    response = render(
        ...
        "proofs/index.html",
        {
            ...
            "verify_count": kpis["verify"] or 0,
            "tracking_count": kpis["tracking"] or 0,
...
def recover(request, pk):
    ...
    with transaction.atomic():
        ...
        proof.note = note
        proof.resolution_source = "COORDINATOR"
        proof.save(update_fields=["status", "recovered_at", "recovery_driver", "confirmed_by", "note", "resolution_source", "updated_at"])
...
def validate_submission(request, submission_pk):
    ...
    with transaction.atomic():
        ...
        if action in {"reject", "request_new"}:
            ...
            proof.resolution_source = "PORTAL_VALIDATED" if submission.source == ProofRecoverySubmission.Source.DRIVER_PORTAL else "COORDINATOR"
            proof.save(update_fields=["status", "recovery_driver", "recovered_at", "confirmed_by", "note", "resolution_source", "updated_at"])
    ...
    from apps.drivers.evaluation import snapshot_driver_scores
    snapshot_driver_scores(driver_ids=[submission.driver_id])
    return redirect(_safe_return(request, proof.pk))
