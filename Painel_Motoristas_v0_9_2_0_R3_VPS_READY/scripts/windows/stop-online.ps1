...
$stopped = $false
foreach ($pidName in @("cloudflared.pid", "server.pid", "scheduler.pid")) {
...
if ($stopped) {
    Write-Host "Painel online, Cloudflare Tunnel e Scheduler SSW encerrados." -ForegroundColor Green
} else {
