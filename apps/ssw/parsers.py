...
def retention_snapshot(row: dict[str, str]) -> RetentionSnapshot:
    ...
    if recovered_at is not None and explicit_retained_at is not None and recovered_at < explicit_retained_at:
        # Dado cronologicamente impossível: não inferimos baixa automática.
        delivered_after_retention = False
        recovered_at = None
