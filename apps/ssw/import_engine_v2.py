...
BATCH_SIZE = 1000
IMPORT_ENGINE_BUILD = "0.3.0.8-proof-state-reconciliation+0.3.0.10-client-identity-safe-upsert"
    ...
    *,
    ...
    try:
        ...
        with transaction.atomic():
            ...
            retained_by_cte: dict[int, tuple[datetime | None, PreparedRow, DeliveryMovement, Client, ClientAddress | None]] = {}
            for r, movement, client, address in zip(prepared, row_movements, row_clients, row_addresses):
                ...
                if r.retained:
                    ...
                    if prev is None or candidate < prev[0]:
                        retained_by_cte[cte.pk] = (candidate, r, movement, client, address)
            ...
            for ctrc, cte in ctes.items():
                ...
                status = latest[2] if latest else fallback_cte_status.get(ctrc, "")
            ...
            for cte_id, (retained_at, r, movement, client, address) in retained_by_cte.items():
                cte = ctes[r.ctrc]
                manifest = manifests[r.manifest_number]
                driver = drivers_by_cpf[r.driver_cpf]
                latest_state = latest_cte.get(cte_id)
                latest_code = latest_state[1] if latest_state else r.ctrc_code
                latest_desc = latest_state[2] if latest_state else r.ctrc_description
                latest_at = latest_state[3] if latest_state else r.ctrc_occurred_at
                ...
                if proof is None:
                    initial_status = RetainedProof.Status.RECOVERED if delivered_after_retention else RetainedProof.Status.WAITING
                    proof = RetainedProof(
                        cte=cte,
                        invoice_number=r.invoice_number,
                        client=client,
                        ...
                        note=_automatic_note("Baixa automática", latest_at) if delivered_after_retention else "",
                        created_at=now,
                        updated_at=now,
                    )
                ...
                changed = False
                # Corrige bases antigas contaminadas pelo fallback = instante da importação.
                ...
                if not manual_recovery and proof.status != RetainedProof.Status.CANCELED:
                    if delivered_after_retention:
                        ...
                        if proof.confirmed_by_id is not None:
                            ...
                            changed = True
                        changed = _replace_automatic_note(proof, _automatic_note("Baixa automática", latest_at)) or changed
                    elif active_retention:
                        # Só reabre uma baixa AUTOMÁTICA se surgiu retenção CTRC mais nova.
                        if proof.status == RetainedProof.Status.RECOVERED and proof.note.startswith(AUTO_SSW_NOTE_PREFIX):
                            if proof.recovered_at is None or latest_at is None or latest_at > proof.recovered_at:
                                ...
                                proof.recovery_driver = None
                                proof.confirmed_by = None
                                _replace_automatic_note(proof, _automatic_note("Retenção reaberta", latest_at))
        ...
        # O pipeline não recalcula dashboards históricos; as views agregam apenas o período solicitado.
