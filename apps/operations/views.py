...
def today(request):
    ...
    cards = build_manifest_cards(
        manifests,
        persist_available=(d == timezone.localdate()),
        operational_date=d,
    )
    classifications = operational_manifest_classification_map(d)
    all_evidence = operational_manifest_evidence_map()
    for card in cards:
...
def deliveries(request):
    ...
    qs = (
        ...
        .select_related("cte", "client", "address", "driver", "manifest", "manifest__vehicle")
    )
    ...
    delivered_filtered = completed_cte_ids(filtered_cte_ids, as_of=end)
...
def geo_neighborhood_geometry_api(request):
    ...
    collection, meta = neighborhood_feature_collection(state, city, districts)
