...
from apps.drivers.models import Driver, DriverPortalAccess
...
def _driver_rows(request, target_date):
    ...
    for manifest in manifests:
        grouped.setdefault(manifest.driver_id, {"driver": manifest.driver, "manifests": []})["manifests"].append(manifest)
    rows = []
    for entry in grouped.values():
        driver = entry["driver"]
        summary = driver_operation_summary(entry["manifests"], target_date)
        phone = normalize_whatsapp_phone(driver.whatsapp_phone)
        access_url = existing_portal_url(request, driver) if driver.active else ""
        last = WhatsAppMessage.objects.filter(driver=driver, operation_date=target_date).order_by("-created_at").first()
        rows.append({
            **entry, "summary": summary, "phone": phone, "portal_url": access_url,
            # O link pode ser criado automaticamente no momento do envio. Assim,
            # abrir esta tela não cria tokens por efeito colateral.
            "ready": bool(driver.active and driver.whatsapp_enabled and is_valid_whatsapp_phone(phone) and public_portal_ready(request)),
            "phone_ready": bool(driver.active and driver.whatsapp_enabled and is_valid_whatsapp_phone(phone)),
            "link_ready": bool(access_url), "last_message": last,
        })
    return rows


def _all_driver_rows(request, target_date, operation_rows=None):
    operation_rows = operation_rows if operation_rows is not None else _driver_rows(request, target_date)
    operation_rows = {row["driver"].pk: row for row in operation_rows}
    rows = []
    for driver in Driver.objects.filter(active=True, is_test=False).order_by("name"):
        op = operation_rows.get(driver.pk)
        phone = normalize_whatsapp_phone(driver.whatsapp_phone)
        last = WhatsAppMessage.objects.filter(driver=driver).order_by("-created_at").first()
        rows.append({
            "driver": driver,
            "phone": phone,
            "phone_ready": bool(driver.whatsapp_enabled and is_valid_whatsapp_phone(phone)),
            "ready": bool(driver.whatsapp_enabled and is_valid_whatsapp_phone(phone) and public_portal_ready(request)),
            "has_operation": bool(op),
            "summary": op["summary"] if op else {"routes": 0, "clients": 0, "movements": 0, "opportunities": 0},
            "last_message": last,
        })
    return rows
...
def center(request):
    ...
    all_driver_rows = _all_driver_rows(request, target_date, rows)
    ...
    return render(request, "messaging/center.html", {
        ...
        "all_driver_rows": all_driver_rows,
        "all_ready_count": sum(1 for r in all_driver_rows if r["ready"]),
        "all_pending_count": sum(1 for r in all_driver_rows if not r["ready"]),
        "pending_rows": pending,
        "bot": bot,
        "recent": recent,
        "planned_manifests": planned,
        "public_portal_ready": public_portal_ready(request),
...
def send_day(request):
    ...
    if not public_portal_ready(request):
        ...
        return redirect(request.POST.get("next") or "whatsapp_center")
    target_date = _date(request)
    rows = _driver_rows(request, target_date)
    queued = 0
    skipped = []
...
def send_manifest(request, pk):
    ...
    if not driver.whatsapp_enabled or not is_valid_whatsapp_phone(phone):
        ...
        return redirect(request.POST.get("next") or "operations_today")
    ...
    if pending_duplicate:
        ...
        return redirect(request.POST.get("next") or f"/operacao/hoje/?date={target_date.isoformat()}")
    portal_url = portal_url_for(request, driver)
